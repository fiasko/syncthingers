...
use crate::process_monitor::{register_process_exit_monitor, ProcessMonitorHandle};
...
impl SyncthingProcess {    
                ...
                if let Some(app_state_weak) = &self.app_state {
                    ...
                    match register_process_exit_monitor(
                        ...
                        Ok(monitor) => {
                            ...
                            log::info!("Successfully registered exit monitor for Syncthing process (PID: {})", child.id());
                        },
                        Err(e) => {
    ...
    pub fn detect_process(exe_path: &str, external_only: bool, app_state: Option<Weak<Mutex<AppState>>>) -> io::Result<Option<Self>> {
        #[cfg(target_os = "windows")]
        {
                                ...
                                if !process_handle.is_null() {
                                    match register_process_exit_monitor(
                                        ...
                                        Ok(monitor) => {
                                            ...
                                            log::info!("Successfully registered exit monitor for external Syncthing (PID: {})", pid);
                                        },
                                        Err(e) => {
                                            log::warn!("Failed to register exit monitor for external Syncthing: {}", e);
                                            // Continue without monitoring - we'll fallback to polling
                                        }
                                    }
                                    ...
                                    log::warn!("Could not open handle to process {} for monitoring", pid);
                                }
                            }
                        }
                    }
